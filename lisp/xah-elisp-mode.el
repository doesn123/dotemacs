...
;; Version: 3.33.20220726224521
;; Created: 23 Mar 2013
;; Package-Requires: ((emacs "25.1"))
;; Keywords: lisp, languages
;; License: GPL v2. Tell your friends to buy a copy.
;; Homepage: http://xahlee.info/emacs/emacs/xah-elisp-mode.html
...
;; • Completion for function names with flex match, for ALL symbols in obarray (this means, including any package you've loaded.). (press TAB after word)
...
;; home page: http://xahlee.info/emacs/emacs/xah-elisp-mode.html
...
;; If you like the idea, please help fund the project. Buy Xah Emacs Tutorial http://xahlee.info/emacs/emacs/buy_xah_emacs_tutorial.html or make a donation. See home page. Thanks.
...
version 2017-01-27 2022-02-11 2022-04-07"
  (interactive)
  (let* (($bds (bounds-of-thing-at-point 'symbol))
    ...
    (setq $resultSym (completing-read "" xah-elisp-all-symbols nil nil $thisSym))
    (delete-region $p1 $p2)
    (goto-char $p1)
    (insert $resultSym)))
...
(define-abbrev-table 'xah-elisp-mode-abbrev-table
  '(("c" "concat" xah-elisp--ahf)
    ...
    ("make-marker" "(make-marker)" xah-elisp--ahf)
    ...
    ("point-marker" "(point-marker)" xah-elisp--ahf)
    ...
    ("set-marker" "(set-marker MARKER▮ POSITION &optional BUFFER)" xah-elisp--ahf)
...
(setq xah-elisp-font-lock-keywords
      (let ()
        `((,(regexp-opt xah-elisp-ampersand-words 'symbols) . 'font-lock-builtin-face)
          ...
          ("\\_<[ξ$x][-_?0-9A-Za-z]*" . 'bold)
          ;; ("\\_<$[-_?0-9A-Za-z]+" . 'bold)
          ;; ("\\_<$[-_?0-9A-Za-z]+" . 'bold)
          ("\\_<φ[-_?0-9A-Za-z]+" . 'xah-elisp-cap-variable)
          ("\\_<@[-_?0-9A-Za-z]+" . 'xah-elisp-at-symbol)
          ("\\_<[A-Z][-_?0-9A-Za-z]*" . 'xah-elisp-cap-variable)
...
(progn
  ...
  (define-key xah-elisp-mode-map
    (if (boundp 'xahemacs-major-mode-leader-key)
      ...
      (kbd "TAB"))
    xah-elisp-leader-map)

  (define-key xah-elisp-leader-map (kbd "TAB") 'xah-elisp-complete-or-indent)
  ...
  (define-key xah-elisp-leader-map (kbd "t") 'transpose-sexps)
...
home page:
URL `http://xahlee.info/emacs/emacs/xah-elisp-mode.html'
